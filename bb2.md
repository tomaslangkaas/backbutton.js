...
*The 1kb standalone router for client-side JavaScript applications*
...
* `backbutton.refresh()` notifies all observers that `location.hash` has changed, while it has not.
* `backbutton.observe()` registers observers to monitor changes in `location.hash`. Calling this function does not create any new history entries.
